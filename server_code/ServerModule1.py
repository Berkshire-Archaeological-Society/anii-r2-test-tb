...
def table_update(type,table):
  ...
  #table.replace(r'^\s+$', None, regex=True,inplace=True) # not sure this works
  #print(table)
  ...
  for i,row in table.iterrows():
    ...
    #print(strip_where, strip_values)
...
def table_insert(type,table):
  ...
  for i,row in table.iterrows():
    ...
    try:
       print(tuple(row))
       ret = cur.execute(sql_cmd,tuple(row))
       conn.commit()
...
def describe_table(table_name):
  #print("In describe_table: ",table_name)
...
def context_get_details(site_id,context_id):
  ...
  with conn.cursor() as cur:
    ...
    #cur.execute("SELECT * FROM Image WHERE ContextId = %s AND SiteId = %s",(context_id,site_id))
    #images = cur.fetchall()
    #print(images)
...
def system_user_insert(email,password,systemrole,status,initials,firstname,lastname):
  ...
  try:
    ...
    logmsg("INFO", msg)
  #except anvil.users.UserExistsError:
  #  msg = "A user with that email " + uemail + " already exists."
  #  logmsg("INFO", msg)
